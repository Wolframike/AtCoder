...
bool istakcode(vector<string> s, int i, int j) {
	regex top(R"(###\..{5})");
	regex midtop(R"(\.\.\.\..{5})");
	regex midbot(R"(.{5}\.\.\.\.)");
	regex bot(R"(.{5}\.###)");
	...
	sfor(x, 3) {
		if (!regex_match(s[i + x].substr(j, 9), top))
			return false;
	}
	if (!regex_match(s[i + 3].substr(j, 9), midtop))
		return false;
	if (!regex_match(s[i + 5].substr(j, 9), midbot))
		return false;
	sfor(x, 3) {
		if (!regex_match(s[i + 6 + x].substr(j, 9), bot))
			return false;
	}
	return true;
}
