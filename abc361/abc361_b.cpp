...
signed main(void) {fast
	...
	int x11 = min(p1[0], p2[0]), y11 = min(p1[1], p2[1]), z11 = min(p1[2], p2[2]);
	int x12 = max(p1[0], p2[0]), y12 = max(p1[1], p2[1]), z12 = max(p1[2], p2[2]);
	int x21 = min(p3[0], p4[0]), y21 = min(p3[1], p4[1]), z21 = min(p3[2], p4[2]);
	int x22 = max(p3[0], p4[0]), y22 = max(p3[1], p4[1]), z22 = max(p3[2], p4[2]);

	yn(!(x12 <= x21 || x22 <= x11 || y12 <= y21 || y22 <= y11 || z12 <= z21 || z22 <= z11));
