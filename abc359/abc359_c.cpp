...
signed main(void) {fast
	...
	pii dir;
	dir.first = (T.first - S.first > 0) ? 1 : -1;
	dir.second = (T.second - S.second > 0) ? 1 : -1;
	int cost = 0;
	int move = min(abs(T.first - S.first), abs(T.second - S.second));
	cost += move;
	S.first += dir.first * move;
	S.second += dir.second * move;
	// cout << S.first << " " << S.second << endl;
	// cout << cost << endl;
	if (S.first == T.first) {
		cost += abs(S.second - T.second);
	} else {
		if (S.first % 2 != T.first % 2) {
			if (S.second % 2 == 0 != S.first % 2 == 0)
				T.first++;
			else
				T.first--;
		}
		cost += abs(S.first - T.first) / 2;
	}
	cout << cost << endl;
