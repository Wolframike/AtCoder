...
class dp_minimize {
	private:
		...
		int threshold;
	public:
		explicit dp_minimize(int N) : dp(), threshold(N) {}
		...
		int solve() {
			...
			dp.resize(threshold + maxweight + 1, inf);
			dp[0] = 0;
			...
			return *min_element(dp.begin() + threshold, dp.end());
		}
};
template <typename T>
...
void detonate(vs &B, int x, int y, int range, int w, int h) {
	sfor(i, h)
		sfor(j, w)
...
signed main(void) {fast
	...
	sfor(i, R)
		sfor(j, C)
			if (isdigit(B[i][j]))		
				detonate(field, i, j, B[i][j] - '0', C, R);
