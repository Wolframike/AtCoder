...
signed main(void) {fast
	...
	vector<item> items(N);
	sfor(i, N) {
		cin >> items[i].price >> items[i].fnum;
		items[i].funcs.resize(items[i].fnum);
		sfor(j, items[i].fnum)
			cin >> items[i].funcs[j];
	}
	...
	for(int i = 0; i < N; i++) {
		for(int j = i + 1; j < N; j++) {
			if (items[i].price >= items[j].price && issubset(items[i].funcs, items[j].funcs)) {
				if (items[i].price > items[j].price || items[i].fnum < items[j].fnum) {
					cout << "Yes" << endl;
					return 0;
				}
			}
			if (items[j].price >= items[i].price && issubset(items[j].funcs, items[i].funcs)) {
				if (items[j].price > items[i].price || items[j].fnum < items[i].fnum) {
					cout << "Yes" << endl;
					return 0;
				}
			}
